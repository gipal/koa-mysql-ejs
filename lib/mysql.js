...
//创建连接池
//在服务器应用程序中通常需要为多个数据库连接创建并维护一个连接池，当连接不再需要时，这些连接可以缓存在连接池中，
//当接收到下一个客户端请求时，从连接池中取出连接并重新利用，而不需要再重新建立连接。
...
let query = function(sql, values) {
	return new Promise((resolve, reject) => {
		pool.getConnection(function(err, connection) { // 从连接池取出连接，err为错误对象，connection为获取到的连接对象
			if(err) {
				reject(err)
			} else {
				console.log('lianjie')
				connection.query(sql, values, (err, rows) => { // 使用连接执行查询
					if(err) {
						reject(err)
					} else {
						resolve(rows)
					}
					connection.release() // 连接不再使用，归还到连接池
				})
			}
		})
	})
